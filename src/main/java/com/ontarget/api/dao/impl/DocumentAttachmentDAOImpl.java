...
@Repository("documentAttachmentDAOImpl")
public class DocumentAttachmentDAOImpl extends BaseGenericDAOImpl<DocumentAttachmentDTO> implements DocumentAttachmentDAO {
	...
	public List<DocumentAttachmentDTO> getByDocumentId(long documentId) {
		List<DocumentAttachmentDTO> documentAttachments = jdbcTemplate.query(OnTargetQuery.documentAttachment.GET_BY_DOCUMENT_ID,
				new Object[] { documentId }, new RowMapper<DocumentAttachmentDTO>() {
					@Override
					public DocumentAttachmentDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
						DocumentAttachmentDTO documentAttachment = new DocumentAttachmentDTO();
						documentAttachment.setDocumentAttachmentId(rs.getInt("document_attachment_id"));
