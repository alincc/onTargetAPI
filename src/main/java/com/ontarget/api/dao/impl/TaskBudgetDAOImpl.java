...
public class TaskBudgetDAOImpl implements TaskBudgetDAO {
	...
	@Override
	public Map<String, Object> getMinStartMaxEndDate(long projectId,
			String costType) throws Exception {
		logger.info("Getting min date and max date for projectId " + projectId
				+ " and cost type:" + costType);
		return jdbcTemplate.queryForMap(
				OnTargetQuery.GET_TASK_COST_MIN_MAX_DATE, new Object[] {
						costType, projectId });
	}
	...
	public Map<TaskDTO, List<TaskEstimatedCost>> getTaskToCostMap(long projectId,
		...
		jdbcTemplate.query(
				...
				new Object[] { costType, projectId }, (resultSet, i) -> {
				...
				Comparator<TaskEstimatedCost> byYear = (o1, o2) -> Integer
						.valueOf(o1.getYear()).compareTo(
								Integer.valueOf(o2.getYear()));
				Comparator<TaskEstimatedCost> byMonth = (o1, o2) -> Integer
		...
		jdbcTemplate
				.query(OnTargetQuery.GET_TASK_PLANNED_ESTIMATED_COST_BY_PROJECT,
						...
						(resultSet, i) -> {
							...
							Map<TaskInterval, TaskEstimatedCost> monthYearCost = taskToCostMap
									.get(task);
							if (monthYearCost == null) {
								monthYearCost = new LinkedHashMap<>();
							}
							monthYearCost.put(new TaskInterval(month, year),
									cost);
		...
		jdbcTemplate
				.query(OnTargetQuery.GET_TASK_COST_BY_TASK,
						...
						(resultSet, i) -> {
							...
							Map<TaskInterval, List<TaskEstimatedCost>> actualsPlannedCostByMonthYear = taskMapMap
									.get(task);
							if (actualsPlannedCostByMonthYear == null) {
								actualsPlannedCostByMonthYear = new LinkedHashMap<>();
								taskMapMap.put(task,
										actualsPlannedCostByMonthYear);
							}
							...
							TaskInterval taskInterval = new TaskInterval(month,
									year);
							List<TaskEstimatedCost> taskEstimatedCosts = actualsPlannedCostByMonthYear
									.get(taskInterval);
							if (taskEstimatedCosts == null) {
								taskEstimatedCosts = new LinkedList<>();
								actualsPlannedCostByMonthYear.put(taskInterval,
										taskEstimatedCosts);
							}

							actualsPlannedCostByMonthYear.get(taskInterval)
									.add(cost);
	...
	public TaskDTO getTaskCostByTask(TaskDTO task) throws Exception {
		...
		jdbcTemplate.query(OnTargetQuery.GET_TASK_COST_BY_TASK,
				new Object[] { task }, new RowMapper<Void>() {
					@Override
	...
	@Override
	public Map<TaskInterval, List<TaskEstimatedCost>> getTaskCostByTaskMonthYear(
			int projectTaskId) throws Exception {
		final Map<TaskInterval, List<TaskEstimatedCost>> taskIntervalListMap = new HashMap<>();

		jdbcTemplate.query(OnTargetQuery.GET_TASK_COST_BY_TASK,
				new Object[] { projectTaskId }, new RowMapper<Void>() {
					@Override
					public Void mapRow(ResultSet resultSet, int i)
							throws SQLException {
						...
						TaskInterval taskInterval = new TaskInterval(month,
								year);
						List<TaskEstimatedCost> taskEstimatedCosts = taskIntervalListMap
								.get(taskInterval);
						if (taskEstimatedCosts == null) {
							taskEstimatedCosts = new LinkedList<>();
						}
						taskEstimatedCosts.add(cost);
						taskIntervalListMap.put(taskInterval,
								taskEstimatedCosts);
