...
public class TaskPercentageDAOImpl implements TaskPercentageDAO {
		...
		jdbcTemplate
				.query(OnTargetQuery.GET_TASK_PERCENTAGE,
						...
						(resultSet, i) -> {
							...
							Map<TaskInterval, TaskPercentage> percentageMapByMonthYear = taskToPercentageMap
									.get(task);
							if (percentageMapByMonthYear == null) {
								percentageMapByMonthYear = new LinkedHashMap<>();
							}
							percentageMapByMonthYear.put(new TaskInterval(
									month, year), percentage);

							/**
							 * if percentage complete is zero for this month
							 * year, use the previous month/year
							 */
							double percentageComplete = percentage
							...
							while (percentageComplete == 0) {
								...
								TaskPercentage lastMonthPercentage = percentageMapByMonthYear
										.get(new TaskInterval(month, year));
								percentageComplete = lastMonthPercentage
										.getTaskPercentageComplete();
							}

							percentage
									.setTaskPercentageComplete(percentageComplete);
							...
							taskToPercentageMap.put(task,
									percentageMapByMonthYear);
							return null;
						});
		return taskToPercentageMap;
	}
		...
		jdbcTemplate.update(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(
				...
				PreparedStatement ps = connection.prepareStatement(
						OnTargetQuery.ADD_TASK_PERCENTAGE_COMPLETE,
						new String[] { "id" });
	...
	public boolean updateTaskPercentageComplete(
		...
		int row = jdbcTemplate.update(
				OnTargetQuery.UPDATE_TASK_PERCENTAGE_COMPLETE, new Object[] {
						taskProgressOfTask.getPercentageComplete(), modifiedBy,
						taskProgressOfTask.getTaskPercentageLogId() });
		if (row == 0) {
			throw new Exception(
					"Unable to update task estimated and planned cost");
		}
		return true;
	}

	@Override
	public boolean expireTaskPercentage(int taskPercentageLogId)
			throws Exception {
		int row = jdbcTemplate.update(
				OnTargetQuery.EXPIRE_TASK_PERCENTAGE_COMPLETE,
				new Object[] { taskPercentageLogId });
		if (row == 0) {
			throw new Exception(
					"Unable to expire task estimated and planned cost");
		}
		return true;
	}
	...
	public List<TaskPercentage> getTaskPercentageByTask(int projectTaskId)
		...
		jdbcTemplate
				.query(OnTargetQuery.GET_TASK_PERCENTAGE_BY_TASK,
						...
						(resultSet, i) -> {
							...
							percentage.setTaskPercentageComplete(resultSet
									.getDouble("percentage_complete"));
							percentage.setCreatedBy(resultSet
	...
	public TaskPercentage getExistingTaskPercentageForTheMonth(int taskId)
		...
		jdbcTemplate.query(OnTargetQuery.GET_TASK_PERCENTAGE_FOR_THIS_MONTH,
				new Object[] { taskId }, (resultSet, i) -> {
					taskPercentage.setId(resultSet
							.getInt("task_percentage_log_id"));
					return null;
				});
