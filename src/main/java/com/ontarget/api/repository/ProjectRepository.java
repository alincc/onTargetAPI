...
public interface ProjectRepository extends JpaRepository<Project, Integer> {
	...
	@Query("select p from Project p JOIN p.projectMemberList pm where pm.user.userId = ?1 and p.projectParentId = 0 and p.type='MAIN_PROJECT' and p.projectStatus !="
			+ OnTargetConstant.ProjectStatus.DELETED)
	...
	@Query("select p from Project p join p.projectMemberList pm where pm.user.userId = ?1 and p.type='PROJECT' and p.projectStatus !="
			+ OnTargetConstant.ProjectStatus.DELETED)
