...
public class AuthenticationDAOTest extends BaseTest {
	...
	public void testGetRegistrationRequest() {
		try {
			UserRegistrationRequest info = authenticationDAO
					.getUserRegistrationRequestInfo(3);
			Assert.assertTrue(info != null);
		} catch (Exception e) {
			logger.error("Error while getting user registration request info.",
					e);
			fail();
		}
	}
	...
	public void testGetUserRegistrationPendingRequest() {
		try {
			List<UserRegistrationRequest> requestList = authenticationDAO
