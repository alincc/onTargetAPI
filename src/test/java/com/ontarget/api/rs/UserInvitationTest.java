...
public class UserInvitationTest extends BaseTest {
	...
	public void testInviteUserIntoNewAccount() {
		...
		userInvitationRequest.setCompanyLogoPath("ontarget.jpg");

		System.out.println("Client request.... \n");
		System.out.println(toJsonString(userInvitationRequest, true));
		Response response = sendRequest("/onTargetInvitation/inviteToNewAccount", userInvitationRequest);
	...
	public void getPendingRequestList() {
		System.out.println("Client request.... \n");
		Response response = sendRequest("/onTargetInvitation/pendingRegistrationRequest", "");
		if (response.getStatus() != 200) {
			throw new RuntimeException("Failed : HTTP error code : " + response.getStatus());
		}
		String output = response.readEntity(String.class);
